...
3. **Фаза 2 RAG.** Механизм цитирования + скрейп 7 страниц готовы (§8.7, §8.8). Дальше по желанию:
   проверить ссылки-источники в проде после деплоя; добрать страницы (колледж, цены сайта, раскрыть FAQ);
   multi-query; при росте нагрузки — +vCPU и включить реранкер (`RERANK_ENABLED=true`).
