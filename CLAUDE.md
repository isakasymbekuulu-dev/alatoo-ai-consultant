...
- **Источники-ссылки (2026-06-22):** механизм цитирования готов. `source_url` берётся из
  ...
  работают как раньше (URL пустой). **Осталось:** наполнить данными с реальными URL (скрейп, §13).
...
- **Осталось (§13.3):** скрейп ключевых страниц alatoo.edu.kg в `data/` с front-matter `source_url`,
  затем переиндексация (CI сделает сам, т.к. менялись `ingestion/`).
...
3. **Фаза 2 RAG.** Механизм цитирования готов (§8.7). Осталось: скрейп ключевых страниц
   alatoo.edu.kg (Bright Data) в `data/` с front-matter `source_url:` → переиндексация; multi-query;
   при росте нагрузки — +vCPU и включить реранкер (`RERANK_ENABLED=true`).
